  ...
  public function makeExport($export) {
    ...
    foreach ($export as $batchId => $dao) {
      ...
      while ($dao->fetch()) {
        ...
        // Get compain name using financial transaction Id.
        if (!empty($dao->financial_trxn_id)) {
          $campaign_name = '';
          $result = civicrm_api3('EntityFinancialTrxn', 'get', [
            'sequential' => 1,
            'financial_trxn_id' => $dao->financial_trxn_id,
            'entity_table' => "civicrm_contribution",
          ]);
          foreach ($result['values'] as $key => $financialTrxn) {
